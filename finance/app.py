...
def buy():
    ...
    if request.method == "GET":
        ...
        if info == None:
            ...
            # return apology("Number can not be alphabetical")
        # elif number.isdigit():
        #     if int(number) < 0:
        #         return apology("Number can not be negative")
        else:
            return redirect("/")
...
def register():
    ...
    if request.method == "GET":
        ...
        if not username:
            ...
            session["user_id"] = db.execute("SELECT id FROM users WHERE username IS ?", username)
