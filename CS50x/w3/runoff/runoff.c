...
bool vote(int voter, int rank, string name)
{
    ...
    // Base case is when number of voters is less than '0' because we actually have a voter with index '0'.
    if (voter < 0)
    {
        return 1;
    }
    vote(voter - 1, rank, name);
    for (int j = 0; j < candidate_count; j++) // This recursion and loop are NOT needed here since there are 2 loops running in main that do the same. The function should only have strcmp.
    {
...
void tabulate(void)
{
    ...
    for (int i = 0; i < voter_count; i++)
    {
        ...
        for (int j = 0; j < candidate_count; j++)
        {
            ...
            {
                // I add a vote to the non-eliminated candidate and break out of the loop, i.e. stop ccheking the remaining preference of this i'th voter.
