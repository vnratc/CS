...
int main(int argc, string argv[])
{
    ...
    for (int i = 0; i < voter_count; i++)
    {
        ...
        // Check for invalid vote. "if (!vote(name))" means "if (vote(name) == 0)" i.e. if no matches found as per function "vote" below. Does "if (vote(name))" mean "if (vote(name) == 1)"???
        if (!vote(name))
        {
            printf("Invalid vote.\n");
        }
...
bool vote(string name)
{
    // Running a loop on to compare names entered by user with names stored in "candidates[i].name" string of our "candidates" struct. Adding 1 to "candidates[i].votes" if names match.
    for (int i = 0; i < candidate_count; i++)
    {
